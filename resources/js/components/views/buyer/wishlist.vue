<template>
    <div class="">
        <div v-if="$store.state.cart.length > 0">
            <div class="mb-3" v-for="(meal, index) in $store.state.cart" :key="index">
                <div class="d-flex">
                    <div class="col-md-3">
                    <!--    <img :src="require('../../../../../public/images/' +meal.image+ '.png')" alt="" class="user_image">-->
                    </div>
                    <div class="col-md-6">
                        <h2>{{meal.name}}</h2>
                        <p>NG₦{{meal.price}}</p>
                    </div>
                    <div class="col-md-3">
                        <div>
                            <a  :click="adjust(-1)">
                                <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-dash-square-fill" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                                    <path fill-rule="evenodd" d="M2 0a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2H2zm2.5 7.5a.5.5 0 0 0 0 1h7a.5.5 0 0 0 0-1h-7z"/>
                                </svg>
                            </a>
                            <a id="myValue">{{ meal.quantity }}</a>
                            <a :click="adjust(1)">
                                <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-plus-square-fill" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                                    <path fill-rule="evenodd" d="M2 0a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2H2zm6.5 4.5a.5.5 0 0 0-1 0v3h-3a.5.5 0 0 0 0 1h3v3a.5.5 0 0 0 1 0v-3h3a.5.5 0 0 0 0-1h-3v-3z"/>
                                </svg>
                            </a>
                        </div>
                        <button class="btn btn-danger mt-3" @click.prevent="removeFromCart(meal)">
                            <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-x-square-fill" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                                <path fill-rule="evenodd" d="M2 0a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2H2zm3.354 4.646a.5.5 0 1 0-.708.708L7.293 8l-2.647 2.646a.5.5 0 0 0 .708.708L8 8.707l2.646 2.647a.5.5 0 0 0 .708-.708L8.707 8l2.647-2.646a.5.5 0 0 0-.708-.708L8 7.293 5.354 4.646z"/>
                            </svg>
                        </button>
                    </div>
                </div><hr>
            </div><hr class="bg-success">
            <div>
                <div class="d-flex justify-content-between">
                    <p>Total:</p>
                    <p>NGN{{ totalPrice }}</p>
                </div>
                <button class="btn btn-success btn-lg">Checkout</button>
            </div>
        </div>
        <div v-else class="navbar-dropdown">
            <a class="navbar-item" href="">
                Cart is empty
            </a>
        </div>
    </div>
</template>

<script>
export default {
    data(){
        return{
        }
    },

    computed: {
    totalPrice() {
        let total = 0;

        for (let meal of this.$store.state.cart) {
            total += meal.totalPrice;
        }

        return total.toFixed(2);
    }
},
methods: {
    adjust(n) {
            this.$store.commit('adjust', n);
        },
    removeFromCart(meal) {
        this.$store.commit('removeFromCart', meal);
    }
}
}
</script>
<style scoped>
    .user_image{
        width: 100px;
        height: 100px;
        border-radius: 4px;
    }
</style>